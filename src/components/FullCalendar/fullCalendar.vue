<template>
  <div class="comp-full-calendar">
    ...
    <div class="full-calendar-body">
      <div class="weeks">
        <strong class="week" :key="dayIndex" v-for="dayIndex in 7">{{ (dayIndex - 1) | localeWeekDay(firstDay, locale) }}</strong>
      </div>
      <div class="dates" ref="dates">
        ...
        <div class="dates-events">
          <div class="events-week" v-for="(week, weekIndex) in currentDates" :key="weekIndex">
            <div class="events-day" v-for="(day, dayIndex) in week" :key="dayIndex" track-by="$index"
                 :class="{'today' : day.isToday,
              'not-cur-month' : !day.isCurMonth}" @click.stop="dayClick(day.date, $event)">
              <p class="day-number">{{day.monthDay}}</p>
              <div class="event-box">
                <event-card :event="event" :date="day.date" :firstDay="firstDay" v-for="(event, eventIndex) in day.events" :key="eventIndex" v-show="event.cellIndex <= eventLimit" @click="eventClick">
        ...
        <div class="more-events" v-show="showMore"
          ...
          <div class="more-body">
            <div class="body-list">
              <div v-for="event in selectDay.events" v-show="event.isShow" class="body-item" @click="eventClick(event, $event)">
                <event-card :event="event" :date="selectDay.date">
                  <template slot-scope="p">
                    <slot name="fc-event-card" :event="p.event"></slot>
                  </template>
                </event-card>
